...
// 1. Define original 3 English blog posts
const originalEn = [
	{
		...
		content: `<p>In modern product design, we see a recurring pattern: a user onboarding flow feels dull, or retention rates start to dip, and the immediate recommendation is: <em>"Let's add points and a leaderboard!"</em></p>
...
<p>To move away from superficial mechanics, we must focus on the <strong>7 Intrinsic Drivers</strong> of the BEM model. For example, rather than placing a leaderboard to force competition, we can design a <em>Sandbox Schema</em> that unlocks <strong>Discovery and Creativity</strong>, or scaffold a journey that taps into <strong>Epic Identity and Purpose</strong>. True gamification is invisible. It operates in the subtle flow of information, the voluntary choice of difficulty, and the emotional resonance of the system.</p>`,
		faq: [
			{
				question: "What is the main problem with lazy gamification?",
				answer: "Lazy gamification superficially overlays extrinsic rewards like points and leaderboards onto a poorly structured experience, which temporarily boosts metrics but ultimately destroys long-term intrinsic motivation."
			},
			{
				question: "What are the three core paradigms of the BEM framework?",
				answer: "The BEM framework structures engagement around three key paradigms: Behavioral (focusing on user intent and KBIs), Experiential (crafting tension and flow using KEIs), and Meaningful (measuring progress and systemic growth via KLIs)."
			}
		]
	},
	{
		...
		content: `<p>When analyzing why games like <em>Tetris</em> or <em>Civilization</em> keep players engaged for hundreds of hours, designers don't look at the ending credits. They look at the <strong>Core Loop</strong>.</p>
...
<p>A major mistake in traditional product design is removing all friction and uncertainty. When a system is 100% predictable, it becomes boring. BEM loops utilize <strong>Tactical Randomness</strong> to set initial conditions, forcing players to think strategically rather than mechanically. This creates anticipation, releases Dopamine naturally upon success, and fosters a robust sense of personal competence.</p>`,
		faq: [
			{
				question: "What is a core loop in product design?",
				answer: "A core loop is the repetitive sequence of micro-interactions that forms the primary user experience. An engaging core loop is critical to sustaining long-term user retention."
			},
			{
				question: "How does the BEM framework structure a playful loop?",
				answer: "A BEM playful loop is structured as an active learning loop with three stages: Voluntary Action, Recognizable Response (immediate high-fidelity feedback), and Strategic Adjustment (updating the mental model)."
			}
		]
	},
	{
		...
		content: `<p>Most corporate training and educational courses suffer from a "completion bias." They reward users simply for sitting through slides and passing a multiple-choice quiz. Once the certificate is issued, the knowledge is forgotten.</p>
...
<ul>
  ...
  <li><strong>Interactive Failure</strong>: Treating mistakes as informative waypoints rather than final punishments. In a good BEM system, failure is just another loop that delivers the critical insights needed to succeed on the next attempt.</li>
		...
		faq: [
			{
				question: "How do BEM progress systems differ from traditional grades?",
				answer: "Traditional grading is punitive, subtracting points from a 100% total and causing threat avoidance. BEM progress systems build up from 0 XP, ensuring every action (even failure) adds value and context."
			},
			{
				question: "What are the four pillars of scaffolding mastery in BEM?",
				answer: "The four structural pillars are Epic Onboarding, Goal Scaffolding (main vs. optional secondary quests), Dynamic Difficulty Adjustment, and Interactive Failure (where mistakes deliver actionable insights)."
			}
		]
	}
];

// 2. Define original 3 Spanish blog posts
const originalEs = [
	{
		slug: 'lazy-gamification-vs-bem',
		...
		content: `<p>En el diseño de productos modernos, vemos un patrón repetitivo: un flujo de registro se siente aburrido o la retención empieza a bajar, y la recomendación inmediata es: <em>"¡Agreguemos puntos y una tabla de clasificación!"</em></p>
...
<ul>
  ...
  <li><strong>Paradigma Experiencial</strong>: Gesta la tensión, el ritmo del recorrido y los bucles de feedback (KEI) para garantizar que la jornada se sienta gratificante por sí misma y en un estado constante de Flow.</li>
...
<p>Para alejarse de las mecánicas superficiales, debemos concentrarnos en los <strong>7 Motivadores Intrínsecos</strong> del modelo BEM. Por ejemplo, en lugar de colocar una tabla de clasificación para forzar la competencia, podemos diseñar un <em>Esquema de Sandbox (Caja de Arena)</em> que active el <strong>Descubrimiento y la Creatividad</strong>, o estructurar un recorrido que se apoye en la <strong>Identidad Épica y Propósito</strong>. La verdadera gamificación es invisible. Opera en el flujo sutil de la información, en la elección voluntaria de la dificultad y en la resonancia emocional del sistema.</p>`,
		faq: [
			{
				question: "¿Cuál es el principal problema de la gamificación perezosa?",
				answer: "La gamificación perezosa superpone de forma superficial recompensas extrínsecas (puntos, medallas, tablas) sobre una experiencia mal estructurada, destruyendo la motivación intrínseca a largo plazo."
			},
			{
				question: "¿Cuáles son los tres paradigmas fundamentales del framework BEM?",
				answer: "Los paradigmas son el Conductual (Behavior, enfocado en KBIs), Experiencial (Experience, enfocado en KEIs y flujo) y de Sentido (Meaning, enfocado en KLIs y evolución)."
			}
		]
	},
	{
		...
		content: `<p>Al analizar por qué juegos como <em>Tetris</em> o <em>Civilization</em> mantienen a los jugadores enganchados durante cientos de horas, los diseñadores no miran los créditos finales. Analizan el <strong>Core Loop (Bucle Principal)</strong>.</p>
...
<p>Un gran error en el diseño de productos tradicionales es eliminar toda la fricción y la incertidumbre. Cuando un sistema es 100% predecible, se vuelve plano. Los bucles BEM utilizan el <strong>Azar Táctico</strong> para establecer las condiciones iniciales del problema, obligando a los jugadores a pensar de forma estratégica en lugar de mecánica. Esto genera anticipación, libera dopamina de forma natural con el éxito y fomenta un sentido de competencia personal altamente robusto.</p>`,
		faq: [
			{
				question: "¿Qué es un bucle principal (core loop) en el diseño de productos?",
				answer: "Es la secuencia de microinteracciones repetitivas que realiza el usuario. Un core loop interesante y satisfactorio es indispensable para garantizar la retención."
			},
			{
				question: "¿Cómo estructura el framework BEM un bucle de aprendizaje lúdico?",
				answer: "Se divide en tres fases activas: una Acción Voluntaria basada en el riesgo, una Respuesta Reconocible del sistema (feedback) y un Ajuste Estratégico del modelo mental del usuario."
			}
		]
	},
	{
		...
		content: `<p>La mayoría de los cursos de formación corporativa y educativa sufren de "sesgo de finalización". Recompensan a los usuarios simplemente por pasar diapositivas y aprobar un cuestionario de opción múltiple. Una vez expedido el certificado, el conocimiento se olvida.</p>
...
<ul>
  ...
  <li><strong>Fracaso Interactivo</strong>: Tratar los errores como puntos de información valiosos en lugar de castigos finales. En un buen sistema BEM, el fracaso es solo otro bucle que proporciona los insights clave necesarios para tener éxito en el siguiente intento.</li>
		...
		faq: [
			{
				question: "¿Cómo se diferencian las calificaciones tradicionales de los sistemas de progreso BEM?",
				answer: "Las calificaciones son punitivas y restan de un total, activando evitación de riesgos. El progreso BEM acumula XP desde cero, tratando los fallos como fuentes valiosas de información."
			},
			{
				question: "¿Cuáles son los cuatro pilares del andamiaje instruccional en BEM?",
				answer: "Son el Onboarding Épico, el Andamiaje de Objetivos (misiones secundarias), el Ajuste Dinámico de Dificultad y el Fracaso Interactivo."
			}
		]
	}
];

// 3. Read processed JSON files
const scratchDir = '/Users/freetoplay/.gemini/antigravity/scratch';
const compiledEn = [...originalEn];
const compiledEs = [...originalEs];

for (let i = 1; i <= 8; i++) {
	const filePath = path.join(scratchDir, `processed_${i}.json`);
	const rawData = fs.readFileSync(filePath, 'utf8');
	const article = JSON.parse(rawData);

	// Strip HTML tags and calculate readTime if undefined
	let readTime = article.readTime;
	if (readTime === undefined) {
		const enContentWithoutTags = article.en.content.replace(/<[^>]*>/g, ' ');
		const wordCount = enContentWithoutTags.trim().split(/\s+/).length;
		readTime = Math.max(1, Math.round(wordCount / 220));
	}

	// Structure English post
	const postEn = {
		slug: article.slug,
	...
	// Map category to Spanish equivalent
	let categoryEs = article.category;
	if (article.category === 'Systemic Design') categoryEs = 'Diseño Sistémico';
	else if (article.category === 'UX & Game Loops') categoryEs = 'UX y Bucles de Juego';
	else if (article.category === 'L&D & Education') categoryEs = 'Aprendizaje y L&D';
	else if (article.category === 'Psychology') categoryEs = 'Psicología';

	// Structure Spanish post
	const postEs = {
		slug: article.slug,
		category: categoryEs,
		readTime: readTime,
	...
	compiledEn.push(postEn);
	compiledEs.push(postEs);
}

// 4. Generate TS content
const outputFileContent = `export interface FAQItem {
	question: string;
	answer: string;
	question_id?: string;
}
...
export const blogData: Record<'en' | 'es', BlogPost[]> = {
	en: ${JSON.stringify(compiledEn, null, 2)},
	es: ${JSON.stringify(compiledEs, null, 2)}
};
`;

const outputPath = '/Users/freetoplay/Dev/Svelte/bem-framework/src/lib/content/blogData.ts';
fs.writeFileSync(outputPath, outputFileContent, 'utf8');
console.log('Successfully wrote compiled blogData.ts with all 11 articles!');
