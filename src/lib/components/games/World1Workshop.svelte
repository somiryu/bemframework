<script lang="ts">
	...
	onMount(() => {
		...
		const existingState = session.player.game_state?.[1] || {};
		...
		session.initConnection(async (event, payload) => {
			if (event === 'slide-sync') {
				...
				activeSlideMode = payload.mode;
			} else if (event === 'student-submit') {
				classSubmissions = {
					...classSubmissions,
				...
				if (!session.isHost) {
					...
					if (!state[1].workshop_completed) {
						...
						await session.updatePlayerGameState(state, {
							avatar: selectedCharacter || session.player.avatar,
							coins: session.player.coins + 25
						});
					}
				}
				onComplete();
	...
	async function submitSlideChoice() {
		...
		await session.updatePlayerGameState(state, {
			...
			...(currentSlide === 5 ? { coins: session.player.coins + 25 } : {})
		});
	}
	...
	const charCounts = $derived.by(() => {
		const counts: Record<string, number> = { sensei: 0, explorador: 0, proposito: 0, curador: 0, estratega: 0, socio: 0, ingeniero: 0 };
		Object.values(classSubmissions).forEach((s: any) => {
	...
	// Slide 3 Waiting analytics: average points for each skill
	const skillAverages = $derived.by(() => {
		const sums: Record<string, number> = { heroismo: 0, poder: 0, explotacion: 0, habilidad: 0, colaboracion: 0, administracion: 0, arte: 0 };
		let count = 0;
		Object.values(classSubmissions).forEach((s: any) => {
			if (s.slide === 2 && s.skills) {
				count++;
		...
		Object.keys(sums).forEach(k => {
			sums[k] = parseFloat((sums[k] / count).toFixed(1));
		});
		return sums;
	});

	// Slide 4 Waiting analytics: collect game favorites and game dislikes
	const virtuesList = $derived.by(() => {
		return Object.values(classSubmissions)
			.filter((s: any) => s.slide === 3 && s.virtues)
			.map((s: any) => ({ name: s.name, alias: s.alias, value: s.virtues }));
	});
	const flawsList = $derived.by(() => {
		return Object.values(classSubmissions)
			.filter((s: any) => s.slide === 3 && s.flaws)
			.map((s: any) => ({ name: s.name, alias: s.alias, value: s.flaws }));
	});

	// Slide 5 Waiting analytics: counts for game preferences
...
<div class="workshop-container">
	...
	<FacilitatorControlPanel
		...
		submittedCount={slideSubmissionsCount}
		isFirstStep={currentSlide === 0}
		isLastStep={currentSlide === 5}
		activeMode={activeSlideMode}
		onReset={handleResetWorkshop}
		onPrev={() => changeSlide(currentSlide - 1, 'actividad')}
		onNext={() => changeSlide(currentSlide + 1, 'actividad')}
		onToggleMode={(mode) => changeSlide(currentSlide, mode)}
	...
	<div class="steps-progress-dots">
		{#each Array(6) as _, i}
			<div class="step-dot" class:active={currentSlide === i} class:host={session.isHost}>
